...
bool sendHTTPRequest(char **server_reply, const char *hostname,
		     const char *message)
{
	//Allocate memory to prepare server reply
	*server_reply =
	    (char *) malloc(sizeof(char) * SERVER_REPLY_LENGTH);
	...
	if (sockfd < 0) {
		fprintf(stderr,
			"Error: sendHttpRequest: error opening socket\n");
		return 0;
	}

	server = gethostbyname(hostname);
	if (server == NULL) {
		fprintf(stderr, "Error: sendHTTPRequest: no such host\n");
		return 0;
	}
	...
	if (connect
	    (sockfd, (struct sockaddr *) &serv_addr,
	     sizeof(serv_addr)) < 0) {
		fprintf(stderr,
			"Error: sendHTTPRequest: connection error\n");
		return 0;
	}
	//Send message to the server
	if (write(sockfd, message, strlen(message)) < 0) {
		fprintf(stderr,
			"Error: sendHTTPRequest: Can't write to socket\n");
		return 0;
	}
	//Read server resposne
	if (read(sockfd, *server_reply, SERVER_REPLY_LENGTH) < 0) {
		fprintf(stderr,
			"Error: sendHTTPRequest: Can't read from socket\n");
		return 0;
	}
#endif
	...
	if ((recv_size =
	     recv(s, *server_reply, SERVER_REPLY_LENGTH,
		  0)) == SOCKET_ERROR) {
...
char *getHypertext(const char *url)
{
	...
	//Form get request
	char *get_request =
	    (char *) malloc(strlen(url_object) + strlen(url_hostname) +
			    strlen(url_object) + 25);
